...
本项目通过 GitHub Actions 实现永久免费、零服务器成本的无人值守晚寝自动签到。工作流预设于北京时间每晚 **21:31:00**（UTC 13:31）准时启动，比本地桌面容灾计划（21:35:00）提前 4 分钟完成首发打卡。
...
3. 在左侧工作流列表中选择 **`AHUT 晚寝自动签到 (每日定时 & 容灾双轨)`**；
...
- **首发打卡（云端 Actions）**：每晚 21:31:00 自动拉起执行。若打卡成功，将写入学校服务端数据，并静默推送到微信；
- **容灾兜底（本地桌面）**：每晚 21:35:00 本地 Windows 任务计划程序拉起浏览器；油猴脚本首先核验当日是否已签到。若检测到云端已打卡成功，看板立即显示绿色 `🎉 今日已签到` 并安全退出，实现零重复提交、零业务冲突的双重保障。
